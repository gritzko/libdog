...
| WHIFF.h | `wh64` packed words (off40 \| id20 \| type4) and `wh128` (key, val) pairs. Used by ULOG idx, keeper LSM index, graf entries. SHA-1 hashlet helpers (40-bit / 60-bit) — the 40/60 twins (`WHIFFHashlet*`, `WHIFFHexFeed*`, `WHIFFHexHashlet*`) are thin wrappers over width-parameterized cores (`whiff_hashlet`/`whiff_hex_feed`/`whiff_hex_hashlet`, chars=10/15). Covered by `test/WHIFF.c`. |
